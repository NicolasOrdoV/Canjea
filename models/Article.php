...
class Article {
    ...
    static public function findArticle($table,$category,$init,$pagination){
        try {
                ...
                WHERE c.Nombre_Categoria LIKE '%$category%' LIMIT :init , :pagination");
...
/// $pagination = al parametro "id" /// ----------/////$item="id_Articulo"
    static public function paginationArticles($table, $init, $pagination){

            try {
                $stmt = Conexion::conectar()->prepare("SELECT a.*,c.Nombre_Categoria  ,u.Nombre as user,u.Correo as correo FROM $table a
                INNER JOIN categoria c ON c.Id_categoria = a.Categoria_Id_Categoria 
                INNER JOIN usuario u on u.Id_Usuario = a.Usuario_Id_Dueno
                LIMIT :init , :pagination ");
                 $stmt -> bindParam(":init",$init, PDO::PARAM_INT);
                 $stmt -> bindParam(":pagination",$pagination, PDO::PARAM_INT);
                 $stmt -> execute();
                 return $stmt -> fetchAll();
    ...
    static public function proveArticlesClothes($table){
        try {
                $stmt = Conexion::conectar()->prepare("SELECT a.*,c.Nombre_Categoria ,u.Nombre as user FROM $table a INNER JOIN categoria c ON c.Id_Categoria = a.Categoria_Id_categoria INNER JOIN usuario u on u.Id_Usuario = a.Usuario_Id_Dueno WHERE c.Id_Categoria=3");
                $stmt->execute();
                return $stmt->fetchAll();
