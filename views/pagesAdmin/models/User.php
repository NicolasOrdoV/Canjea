    ...
    static public function showUser($table,$item,$value){
        try {
            if ($item==null && $value==null) {
                $stmt = Conexion::conectar()->prepare("SELECT u.*,r.TipoRol as rol ,b.Nombre_barrio as street FROM $table u INNER JOIN rol r ON r.IdRol = u.IdRol INNER JOIN barrio b on b.Id_barrio = u.Id_barrio  WHERE u.IdRol = 2 ORDER BY u.Id_Usuario DESC ");
                $stmt->execute();
